...
def build_simbench_net(
        simbench_network_name, gen_scaling=1.0, load_scaling=2.0, voltage_band=0.05, max_loading=80, *args, **kwargs):
    ...
    # plot.simple_plot(net, plot_sgens=False, plot_loads=False, plot_line_switches=False, plot_gens=True,)
    return net, profiles
...
def repair_simbench_profiles(net, profiles):
    ...
    for type_act in profiles.keys():
        ...
        is_equal = profiles[type_act].max(axis=0) == profiles[type_act].min(axis=0)
...
def set_constraints_from_profiles(net, profiles):
    ...
    load_gen_diff = profiles[('load', 'p_mw')].sum(axis=1) - profiles[('sgen', 'p_mw')].sum(axis=1)
